...
当前 MVP 刻意保持保守：`doctor` 只提出建议，`prepare` 只生成预览，`create` 才会写入 packet files。它**不会**修改 Hermes core、不会自动重启 session、不会解析完整 Hermes transcript、不会启动新 agent、不会云端同步、不提供 dashboard，也不会 hidden writes。
...
不要把它当成 secret vault、完整 transcript archive、云端同步工具，或自动 session manager。
...
单独输入 `/handoff` 或 `/handoff help` 只会显示 help，不会创建信息不足的 packet。Plugin `auto_task_state` 是选择性功能，边界与 CLI `--auto-task-state` 相同。
...
- 维持当前产品事实：MVP 只 recommend/preview/create/resume handoff packets；不自动重启 session、不解析完整 transcripts，也不 hidden writes。
