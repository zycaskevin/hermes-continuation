...
当前 MVP 刻意保持保守：它**不会**修改 Hermes core、不会自动重启 session、不会解析完整 Hermes transcript、不会启动新 agent、不会云端同步，也不提供 dashboard。
...
- 维持当前产品事实：MVP 只 create/resume handoff packets；不自动重启 session，也不解析完整 transcripts。
