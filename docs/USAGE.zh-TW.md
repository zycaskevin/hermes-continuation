...
目前 MVP 刻意保持保守：`doctor` 只提出建議，`prepare` 只產生預覽，`create` 才會寫入 packet files。它**不會**修改 Hermes core、不會自動重啟 session、不會解析完整 Hermes transcript、不會啟動新 agent、不會雲端同步、不提供 dashboard，也不會 hidden writes。
...
- 維持目前產品事實：MVP 只 recommend/preview/create/resume handoff packets；不自動重啟 session、不解析完整 transcripts，也不 hidden writes。
